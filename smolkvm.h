...
/* Uppercase `src` into `dst` as a valid C identifier (non-alnum -> '_') */
static void __smolkvm_ident_upper(char *dst, size_t dstsz, const char *src)
{
	size_t i = 0;
...
/*
 * Emit a C header giving every MMIO register's physical address, built from the
 * devices' register metadata. `out` may be NULL to write to stdout. Devices
 * without a register map are skipped.
 */
void smolkvm_dump_register_header(const struct smolkvm_vm *vm, FILE *out)
{
	unsigned int i;
	...
	fprintf(out, "/* Generated by smolkvm_dump_register_header() -- do not edit */\n");
	fprintf(out, "#ifndef SMOLKVM_REGISTERS_H\n#define SMOLKVM_REGISTERS_H\n\n");
	...
	for (i = 0; i < SMOLKVM_ARRAYSIZE(vm->mmioregions); i++) {
		...
		if (!m || !m->regs || m->num_regs == 0)
			continue;

		__smolkvm_ident_upper(dev, sizeof(dev), m->name);

		fprintf(out, "/* %s @ 0x%llx */\n", m->name, (unsigned long long)m->phys);

		snprintf(macro, sizeof(macro), "SMOLKVM_%s_BASE", dev);
		fprintf(out, "#define %-40s 0x%016llxULL\n", macro, (unsigned long long)m->phys);
		snprintf(macro, sizeof(macro), "SMOLKVM_%s_LEN", dev);
		fprintf(out, "#define %-40s 0x%016llxULL\n", macro, (unsigned long long)m->len);

		for (r = 0; r < m->num_regs; r++) {
			const struct smolkvm_mmio_reg *rg = &m->regs[r];
			char reg[64];

			__smolkvm_ident_upper(reg, sizeof(reg), rg->name);
			snprintf(macro, sizeof(macro), "SMOLKVM_%s_%s", dev, reg);
			fprintf(out, "#define %-40s 0x%016llxULL  /* +0x%llx, %llu byte%s */\n",
				macro,
				(unsigned long long)(m->phys + rg->offset),
				(unsigned long long)rg->offset,
				(unsigned long long)rg->size,
				rg->size == 1 ? "" : "s");
		}
	...
	fprintf(out, "/* low memory: identity-mapped RAM covering the legacy \"BIOS area\" */\n");
	fprintf(out, "#define %-40s 0x%016llxULL\n", "SMOLKVM_LOWMEMORY_BASE",
		(unsigned long long) SMOLKVM_LOWMEMORY_PHYS_START);
	fprintf(out, "#define %-40s 0x%016llxULL\n", "SMOLKVM_LOWMEMORY_LEN",
		(unsigned long long) SMOLKVM_LOWMEMORY_SZ);
	fprintf(out, "\n");
